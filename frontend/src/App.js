      ...
      <aside style={styles.sidebar}>
        ...
        <div style={styles.userSection}>
          ...
          <div>
            ...
            <div style={styles.userRole}>{isAdmin ? 'Yönetici' : 'Personel'}</div>
          </div>
        </div>

        <nav style={styles.nav}>
          <button onClick={() => setActivePage('dashboard')} style={activePage === 'dashboard' ? styles.activeLink : styles.link}>
            <FaChartLine /> Özet Panel
          </button>
          <button onClick={() => setActivePage('products')} style={activePage === 'products' ? styles.activeLink : styles.link}>
            <FaBox /> Ürünler
          </button>
          <button onClick={() => setActivePage('categories')} style={activePage === 'categories' ? styles.activeLink : styles.link}>
...
const styles = {
  appLayout: { display: 'flex', background: '#f8fafc', minHeight: '100vh', fontFamily: '"Inter", sans-serif' },
  sidebar: { width: '260px', background: '#fff', height: '100vh', position: 'fixed', left: 0, top: 0, padding: '40px 24px', display: 'flex', flexDirection: 'column', borderRight: '1px solid #f1f5f9', zIndex: 100 },
  logo: { fontSize: '24px', fontWeight: '800', color: '#1e293b', marginBottom: '40px' },
  userSection: { display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', background: '#f8fafc', borderRadius: '16px', marginBottom: '32px' },
